...
from jaxopt.tree_util import tree_where
...
def _binary_replace(replace_bit, original_dict, new_dict, keys=None):
  ...
  for key in keys:
    ...
    #out[key] = tree_where(replace_bit, new_dict[key], original_dict[key])
  return out
...
def _zoom(restricted_func_and_grad, wolfe_one, wolfe_two, a_lo, phi_lo,
  ...
  def body(state):
    ...
    g_j = tree_map(lambda g_j_leaf, g_star_leaf:
                   g_j_leaf.astype(g_star_leaf.dtype), g_j, state.g_star)
  ...
  state = jax.lax.while_loop(lambda state: (~state.done) & (~pass_through) & (~state.failed),
                         body,
                         state)
...
def line_search(value_and_grad_fun, xk, pk, old_fval=None, gfk=None, c1=1e-4, c2=0.9, maxiter=20):
  """Inexact line search that satisfies strong Wolfe conditions.

  Algorithm 3.5 from Wright and Nocedal, 'Numerical Optimization', 1999, pg. 59-61

  Args:
    value_and_grad_fun: function returning value and grad
    x0: initial guess.
  ...
  def restricted_func_and_grad(t):
    phi, g = value_and_grad_fun(tree_add_scalar_mul(xk, t, pk))
  ...
  state = jax.lax.while_loop(lambda state: (~state.done) & (state.i <= maxiter) & (~state.failed),
                         body,
                         state)
...
class ZoomLineSearch:
  ...
  def run(self,
    ...
    Args:
      init_stepsize: initial estimate of the step size.
    ...
    fun = lambda p: self._value_and_grad_fun(p, *args, **kwargs)

    res = line_search(fun, xk=params, pk=descent_direction, old_fval=value,
                      gfk=grad, c1=self.c1, c2=self.c2, maxiter=self.maxiter)
